...
interface CompanyCostsManagerProps {
  companyConfig: PrefixConfig[];
  onConfigChange: (config: PrefixConfig[]) => void;
}

const CompanyCostsManager: React.FC<CompanyCostsManagerProps> = ({ companyConfig, onConfigChange }) => {
  ...
  const handleAdd = () => {
    if (newPrefix.prefix && newPrefix.category && newPrefix.description && newPrefix.costPerMinute !== undefined) {
      const updated = [...companyConfig, newPrefix as PrefixConfig];
  ...
  const handleEdit = (prefix: PrefixConfig, field: keyof PrefixConfig, value: any) => {
    const updated = companyConfig.map(p => p.prefix === prefix.prefix ? { ...p, [field]: value } : p);
    onConfigChange(updated);
  };

  const handleDelete = (prefixToDelete: string) => {
    const updated = companyConfig.filter(p => p.prefix !== prefixToDelete);
    onConfigChange(updated);
  };
    ...
    <Card>
      <CardHeader>
        <div className="flex justify-between items-center">
          <CardTitle>Costi azienda (€/min per prefisso)</CardTitle>
      ...
      <CardContent>
        <Table>
          ...
          <TableBody>
            {companyConfig.map((prefix) => (
              <TableRow key={prefix.prefix}>
                <TableCell className="font-mono">
                  {editingId === prefix.prefix ? (
                    <Input value={prefix.prefix} onChange={(e) => handleEdit(prefix, 'prefix', e.target.value)} className="w-20" />
                  ) : (
                    prefix.prefix
                  )}
                </TableCell>
                <TableCell>
                  {editingId === prefix.prefix ? (
                    <Select value={prefix.category} onValueChange={(value) => handleEdit(prefix, 'category', value)}>
                ...
                <TableCell>
                  {editingId === prefix.prefix ? (
                    <Input value={prefix.description} onChange={(e) => handleEdit(prefix, 'description', e.target.value)} className="w-32" />
                  ) : (
                    prefix.description
                  )}
                </TableCell>
            ...
            {isAdding && (
              <TableRow>
                <TableCell>
                  <Input placeholder="Es: 800" value={newPrefix.prefix || ''} onChange={(e) => setNewPrefix({ ...newPrefix, prefix: e.target.value })} className="w-20" />
                </TableCell>
                <TableCell>
                  <Select value={newPrefix.category} onValueChange={(value) => setNewPrefix({ ...newPrefix, category: value as any })}>
...
export default CompanyCostsManager;
